...
export default {
  config: {
    wrapWith: {
      type: 'string',
      enum: [
        wrapConfig.WRAP_NONE,
        wrapConfig.WRAP_SPACES,
        wrapConfig.WRAP_NEW_LINES,
      ],
      description: 'Wrap commented code with',
      default: wrapConfig.WRAP_NONE
    }
  },
  activate: () => {
...
function toggle() {
  ...
  buffer.transact(() => {
    for (i in selections) {
      const wrapWith = atom.config.get('block-comment-plus.wrapWith') || false;
      ...
      // defaults to Handlebar based comments
      let commentStart = '{{!--';
      let commentEnd = '--}}';
      let addNewLine = wrapWith === wrapConfig.WRAP_NEW_LINES;

      if (wrapWith === wrapConfig.WRAP_SPACES) {
        commentStart += ' ';
